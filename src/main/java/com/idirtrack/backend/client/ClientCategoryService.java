...
public class ClientCategoryService {
    ...
    // Helper method to retrieve a ClientCategory by ID and throw NotFoundException
    // if not found
    ...
    public MyResponse getCategoriesForDropdown() {
        List<ClientCategoryDto> clientCategories = clientCategoryRepository.findAll()
                .stream()
                .map(category -> ClientCategoryDto.builder()
                        .id(category.getId())
                        .name(category.getName())
