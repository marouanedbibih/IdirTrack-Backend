...
public interface SubscriptionRepository extends JpaRepository<Subscription, Long> {
        ...
        @Query("SELECT s FROM Subscription s WHERE s.endDate >= :dateEndFrom AND s.endDate <= :dateEndTo")
        Page<Subscription> filterSubscriptionsByDateEnd(
                        @Param("dateEndFrom") LocalDate dateEndFrom,
                        @Param("dateEndTo") LocalDate dateEndTo,
                        Pageable pageable);
